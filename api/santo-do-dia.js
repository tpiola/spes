...
const FALLBACKS={'08-06':'São Hormisda, papa','08-07':'SS. Sisto II, papa, e Companheiros, mártires'};
...
export default async function handler(_req,res){
  ...
  try{const response=await fetch(sourceUrl,{headers:{'user-agent':'SPES/1.0 (+https://spes.blog)'}});if(response.ok){const html=await response.text();const headings=[...html.matchAll(/<h[2-4][^>]*>([\s\S]*?)<\/h[2-4]>/gi)].map(m=>clean(m[1])).filter(v=>v.length>2&&v.length<120&&!/Santo do dia/i.test(v));const candidate=headings.find(v=>/^(?:SS?|BB?)\.\s|^(?:São|Santa|Santo|Santos|Santas)\b/i.test(v));if(candidate)name=candidate}}catch(_){}
